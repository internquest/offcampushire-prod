...
const Underneath = ({ descRef, underref }) => {
    ...
    const movement = useTransform(scrollYProgress, [0.16, 0.55], [-1220, 3140])

    movementbox = useTransform(scrollYProgress, [0.16, 0.55], [-1025, 1025])
    ...
    return (
        <motion.section ref={underref} style={{ y: (scrollYProgress.get().toFixed(2) > 0.16 ? transfromY : 0) }} className={` z-[-1] relative`}>
            ...
            <motion.div style={{ y: movementboxvalue }} className="mx-auto  bg-cover  bg-top  flex flex-col gap-3 items-center justify-start w-full xl:w-[1280px]  md:h-[572px] mt-14 pt-10 px-3 mb-28 md:mb-96">
