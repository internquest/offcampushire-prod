        ...
        <>
            {
                words.map((word, index) => (
                    <span key={index} className='clipping inline-block bg-inherit '>
                        ...
                        }} ref={wordRef} className={` ${isclip ? ' bg-gradient-to-br from-[#6161A6] to-[#252540] bg-clip-text text-transparent [-webkit-background-clip:text]' : ''}transition-transform inline-block`}>{word}&nbsp;</motion.span>
                    </span>
                ))
            }
        </>
